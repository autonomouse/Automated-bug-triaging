...
import requests
import subprocess
#from client.weebl_python2 import utils
#from client.weebl_python2.exception import UnexpectedStatusCode
...
class Weebl(object):
    ...
    def bug_occurrence_exists(self, bug_uuid, regex):
        bug_occurrence_instances = self.get_instances("bug_occurrence")
        if bug_occurrence_instances != []:
        bug_uuids = [bugocc.get('uuid') for bugocc in bug_occurrence_instances
                  if bug_uuid in bugocc['pipeline'] and regex in 
                  bugocc['job_type']]
        return True if bug_uuid in bug_uuids else False
    ...
    def create_bug_occurrence(self, build_number, job_type, pipeline, regex):
        if self.bug_occurrence_exists(build_number, job_type, pipeline, regex):
            return
            
        # Get build uuid from number:
