...
class CLI(Common):
    ...
    def cli(self):
        ...
        if opts.run_remote:
            ...
            self.run_remote = cfg.get('DEFAULT', 'run_remote')
        ...
        if opts.keep_data:
            ...
            self.keep_data = cfg.get('DEFAULT', 'keep_data').lower() in ['true', 'yes']
...
class Jenkins(Common):
    ...
    def connect_to_jenkins(self):
        ...
        try:
            self.jenkins_api = JenkinsAPI(baseurl=url, cookies=cookies,
                                          netloc=netloc)
...
class TestCatalog(Common):
    ...
    def open_bug_database(self):
        ...
        elif self.cli.database in [None, 'None', 'none', '']:
    ...
    def connect_to_testcatalog(self):
        ...
        if self.cli.run_remote:
            LOG.info("Fetching cookies for %s" % self.cli.tc_host)
            cookies = pycookiecheat.chrome_cookies(self.cli.tc_host)
