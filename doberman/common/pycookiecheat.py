...
Use your browser's cookies to make grabbing data from
login-protected sites easier.  Intended for use with
Python Requests http://python-requests.org

Accepts a URL from which it tries to extract a domain. If you
want to force the domain, just send it the domain you'd like
to use instead.
...
def chrome_cookies(url):
    ...
    # If running Chromium on Linux
        ...
        if os.path.exists(googlechrome):
            cookie_file = googlechrome
        elif os.path.exists(chromium):
            cookie_file = chromium
        else:
            raise Exception("Can't find Google-Chrome nor Chromium cookies.")
    else:
    ...
    # Part of the domain name that will help the sqlite3 query pick it
